...
(define (index-core index frame)
  ...
  (when (test frame '{%mnemonics wikid})
    (let ((v (get frame '{%mnemonics wikid})))
      (index-frame index frame '%id v)
      (index-frame index frame '%id (downcase v))))
  (when (or (test frame '%id) (test frame 'type 'slot)
	    (test frame 'type 'get-methods)
	    (symbol? (get frame '%id)))
    (let ((id (get frame '%id)))
      (index-frame index frame '%id id)
      (index-frame index frame '%id (downcase id))))
  (index-frame index frame '%ids)
  (index-frame index frame '%ids (downcase (get frame '%ids)))
