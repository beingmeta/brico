...
(define (copy-lexslots wikid brico index opts)
  ...
  (let* ((wds #[]) (norms #[]) (glosses #[])
    ...
    (let ((cur-wds (get brico '%norms))
      ...
      (when (and index (oid? brico) (not skip-index))
...
(define (wikid/copy! wikid frame (opts #f) (index wikid.index) (template #f) (unmapped {}))
  ...
  (when template
	...
	(when doindex (index-frame wikid.index frame key add)))))
  (when (and index (oid? frame))
  ...
  (cond ((%test frame 'sensecat))
	((%test frame @?genls)
	 ...
	 (when doindex (index-frame index frame '{sensecat type}))))
  (when doindex
